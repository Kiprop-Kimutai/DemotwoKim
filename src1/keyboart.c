...
void register_ben(int argc, char *argv[]){
	...
	if(fplib_test(0,fp_array))
	{
		...
		//printf("\n%s Personal details \n",cJSON_Print(txToPosted));
		//printf("%s Bio details  \n",myBenf.fingerprint);
		//peformCardOperations(cJSON_Print(txToPosted),myBenf.fingerprint);
		//peformCardOperations(0,0);
		//if(createApplicationAndFileAndWriteData(cJSON_Print(txToPosted),myBenf.fingerprint))
		if(createApplicationAndFileAndWriteData(txToPosted,fp_array))
		{
