	...
	if(ret==0){
		...
		if(ret==0){
			...
			if(ret==0){
				...
				printf("Error writing biodata to card \n");
				return -1;
			}
						...
						//select app and authenticate
						 ...
						 if(authstatus==0){
							 ...
							 case 1:
								...
								//break;
							 case 2:
								 writeops = writefile(fd,keyno,app0,benapp2.AID,authkey,transactiondata,personaldetails);
...
int readfile(int fd,uint8_t keyno,char MF[3],char APP[3],char *authkey,char *response[400] , BYTE * fingerprint[400]){
	int ret,ret1,authstatus;
	desfire_appinfo benapp2;
	(benapp2).AID[0]=0x11;
	(benapp2).AID[1]=0x00;
	...
	if(ret>=0){
		...
		ret =  desfire_read_data(fd,0,0x00,0,100,&outlen,personaldetails);
		if(ret>=0){
			printf("data out ...\n");
			printf("biodata out %s\n",personaldetails);
		...
		ret =  desfire_read_data(fd,1,0x00,0,400,&outlen,biodetails);
		if(ret >= 0){
			printf("data out ...\n");
			printf("biodata out %s\n",biodetails);
			//strcpy(*(fingerprint+0),biodetails);
			//fingerprint[0] = biodetails;
			//return 0;
		}
		else{
			printf("Failed to read card with status ret %d corresponding to errno %d\n",ret,errno);
	...
	if(ret>=0){
		...
		if(personaldetails != ""){
			...
			if(ret>=0){
				...
				printf("Failed to write card with status ret %d corresponding to errno %d\n",ret,errno);
				mif_close(fd);
				return -1;
			}
		}
...
char * readcarduid(void){
	...
	if(fdd>=0){
		printf("mifare open success\n");
		 ret = desfire_select_application(fdd,app0);
		ret = desfire_authenticate_aes(fdd,keyno,authkey);
		if(ret>=0){
			...
			printf("card authentication failed...with result %d\n %d\n",ret,errno);
			return "";
		}
