...
int fp_counter;
int fplib_test(int option, BYTE * fp_pointer)
{
	//option 0 = capture
	//Option 1  = compare
	long err;
	...
	if (err == SGFDX_ERROR_NONE)
	{
		...
		if(option == 1)
		{
			...
			while(n<3)
			{
				...
				if (err != SGFDX_ERROR_NONE)
				{
					message_display_function(1,"","Error", "Please note that we could not fetch image from finger print device",  (char *)NULL);
					kb_getkey();
					err = SGFPM_CloseDevice(hsgfplib);
					err = SGFPM_Terminate(hsgfplib);
					return 0;
				...
				//printf("FP size::%d\n",strlen(minutiaeBuffer1));
				//strcpy(fp_pointer,minutiaeBuffer1);
				printf("FP size::%d\n",strlen(fp_pointer));
				if (err == SGFDX_ERROR_NONE)
				{
					///////////////////////////////////////////////
		...
		else
		{
			...
			message_display_function(1,"","Fetching Finger Print","Press Enter after placing finger", (char *)NULL);
							...
							if (err == SGFDX_ERROR_NONE)
							{
								...
								int x = 0;
								while(x<400){
									printf("%02X",myBenf.fingerprint[x]);
									x++;
								}
